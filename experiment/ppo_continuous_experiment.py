...
class ExperimentPPOContinuous:
    ...
    def run_baseline(self, agent, trial):
        ...
        exploration = GaussianExploration(0, 0.01, step_limit)

        while steps < step_limit:
            state0 = self.process_state(self._env.reset()).unsqueeze(0)
            done = False
            train_ext_reward = 0
            train_steps = 0
            ...
            while not done:
                ...
                next_state, reward, done, info = self._env.step(action0.squeeze(0).numpy())
                state1 = self.process_state(next_state).unsqueeze(0)
                mask = 1
                if done:
                    mask = 0
