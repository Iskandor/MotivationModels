...
class ExperimentPPO:
    ...
    # def run_fm_model(self, agent, trial):
    #     config = self._config
    #     trial = trial + config.shift
    #
    #     step_limit = int(config.steps * 1e6)
    ...
    #
    #         print(
    #             'Run {0:d} step {1:d} training [ext. reward {2:f} int. reward {3:f} steps {4:d} ({5:f})  mean reward {6:f}]'.format(
    #                 trial, steps, train_ext_reward, train_int_reward, train_steps, train_int_reward / train_steps,
    #                 reward_avg.value()))
    #         print(bar)
    #
    #     agent.save('./models/{0:s}_{1}_{2:d}'.format(self._env_name, config.model, trial))
