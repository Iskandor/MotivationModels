...
class FWDAnalytic:
    ...
    def __new__(cls):
        if cls._instance is None:
            print('Creating the object FWDAnalytic')
            cls._instance = super(FWDAnalytic, cls).__new__(cls)
    ...
    def update(self, **kwargs):
        ...
        if 'loss_fwd' in kwargs:
            if self.global_step not in self.loss_fwd:
                self.loss_fwd[self.global_step] = []
            self.loss_fwd[self.global_step].append(kwargs['loss_fwd'].cpu())
    ...
    def finalize(self):
        ...
        self.loss_fwd = self._finalize_value(self.loss_fwd, ['step', 'val'], mode='mean_step')
