...
class MetaCriticRNDMotivation:
    ...
    def train(self, indices):
        if indices:
            ...
            self._optimizer.zero_grad()
            loss = self.network.loss_function(states)
            loss.backward()
            self._optimizer.step()

    def error(self, state0):
        error, error_estimate = self.network.error(state0)
        return error, error_estimate
    ...
    def reward(self, state0):
        ...
        error, error_estimate = self.error(state0)
