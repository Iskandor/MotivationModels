...
def test(config, path):
    env = WrapperAtari(gym.make('PrivateEye-v0'))
    input_shape = env.observation_space.shape
    action_dim = env.action_space.n

    experiment = ExperimentPPO('PrivateEye-v0', env, config)
...
def run_baseline(config, trial):
    ...
    if config.n_env > 1:
        ...
        for i in range(config.n_env):
            env_list.append(WrapperAtari(gym.make('PrivateEye-v0')))

        print('Start training')
        experiment = ExperimentNEnvPPO('PrivateEye-v0', env_list, config)
    else:
        experiment = ExperimentPPO('PrivateEye-v0', env, config)
        experiment.add_preprocess(encode_state)
