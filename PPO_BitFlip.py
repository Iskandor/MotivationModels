...
from agents.PPOBitFlipAgent import PPOBitFlipAgent, PPOBitFlipRNDAgent
...
def run_baseline(config, trial):
    ...
    env = MultiEnvParallel([gym.make('gym_bitflip:bitflip-8-v0') for _ in range(config.n_env)], config.n_env, config.num_threads)
...
def run_rnd_model(config, trial):
    print('Creating {0:d} environments'.format(config.n_env))
    env = MultiEnvParallel([gym.make('gym_bitflip:bitflip-12-v0') for _ in range(config.n_env)], config.n_env, config.num_threads)
