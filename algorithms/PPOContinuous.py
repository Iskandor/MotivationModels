...
class PPOContinuous:
    ...
    def _train(self):
        ...
        for epoch in range(self._ppo_epochs):
            for batch_ofs in range(0, len(trajectory), self._batch_size):
                ...
                dist_v = self._agent.dist(states_v)
                logprob_pi_v = dist_v.log_prob(actions_v)
    ...
    def calc_log_probs(self, states, actions):
        dist = self._agent.dist(states[0:self._batch_size])
        log_probs = dist.log_prob(actions[0:self._batch_size])
        for batch_ofs in range(self._batch_size, self._trajectory_size, self._batch_size):
            batch_l = min(batch_ofs + self._batch_size, self._trajectory_size)
            dist = self._agent.dist(states[batch_ofs:batch_l])
            log_probs = torch.cat([log_probs, dist.log_prob(actions[batch_ofs:batch_l])], dim=0)
