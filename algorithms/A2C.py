...
class A2C:
    ...
    def train(self, state0, log_prob, reward, done):
        state_value = self._agent.value(state0)
        self._trajectory.append((log_prob, state_value))
        ...
        if done or len(self._trajectory) == self._batch_size:
            ...
            loss = policy_loss * self._actor_loss_weight + value_loss * self._critic_loss_weight
            loss.backward()
